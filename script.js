...
function playingGame() {
  if (isPlaying) {
    ...
    if (randomNum === 1 && scorePlayer < 11) {
      randomNum = 11;
    }

    if (cardsPulled[0] === 1 && cardsPulled[1] === 1) {
      blackJackWin();
    }

    if (cardsPulled[0] === 1 && cardsPulled[1] === 11) {
      blackJackWin();
    }

    if (cardsPulled[0] === 11 && cardsPulled[1] === 1) {
      blackJackWin();
    }
  ...
  if (isPlaying) {
    ...
    if (cardsPulled.length < 2) {
      ...
      if (scoreDealer > scorePlayer) {
        // 1. GLAVNI
        if (scoreDealer === 21) {
          alert("Dealer got blackjack! You lost :( 1");
          sound2.play();
          score.losses++;
          lossesPlayerEl.textContent = score.losses;
          isPlaying = false;
        ...
        alert("He got blackjackkkkkkkkkk 2"); // AKO DOBIJE 2 KECA NA POCETKU
        score.losses++;
        lossesPlayerEl.textContent = score.losses;
        isPlaying = false;
      } else if (scorePlayer > scoreDealer && scoreDealer < 22) {
        // 2. GLAVNI
        drawDealerCard(); // vuce 3. kartu
        scoreDealer += randomNum;
        scoreDealerEl.textContent = scoreDealer;
        ...
        if (scorePlayer > scoreDealer && scoreDealer < 22) {
          ...
          if (scoreDealer > scorePlayer && scoreDealer < 22) {
            ...
            alert("Dealer got blackjack 4");
            sound2.play();
            score.losses++;
            lossesPlayerEl.textContent = score.losses;
            isPlaying = false;
          } else if (scoreDealer === scorePlayer) {
            alert("Its draw! 2"); //ovde bug! 3 10 3 1 ne prikazuje sledecu sliku
            score.draws++;
            drawsPlayerEl.textContent = score.draws;
            isPlaying = false;
          }
          if (scorePlayer > scoreDealer && scoreDealer < 22) {
            drawDealerCard(); // vuce 5. kartu
            scoreDealer += randomNum;
            scoreDealerEl.textContent = scoreDealer;
            if (scoreDealer > scorePlayer && scoreDealer < 22) {
              alert("You lost :( 3");
